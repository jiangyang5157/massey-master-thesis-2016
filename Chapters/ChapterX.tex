...
Then, we have to verify if the collision is inside of the quadrangle by putting \emph{t} back to \ref{equ:ray-plane-intersection}, and the \emph{t} is valid only if:

\[
\begin{array}{lr}
\mu = \sqrt{(\overrightarrow{P} - \overrightarrow{S1}) \cdot (\overrightarrow{S2} - \overrightarrow{S1}))} \in [\,0,\,\norm{\overrightarrow{S2} - \overrightarrow{S1}}\,] \\
\nu = \sqrt{(\overrightarrow{P} - \overrightarrow{S1}) \cdot (\overrightarrow{S3} - \overrightarrow{S1}))} \in [\,0,\,\norm{\overrightarrow{S3} - \overrightarrow{S1}}\,] 
\end{array}
\]
