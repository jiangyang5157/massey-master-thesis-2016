...
Third, remove \code{$\triangle$ABC} in the vertex indexes list, add \code{$\triangle$ADE}, \code{$\triangle$DBF}, \code{$\triangle$EFC}, and \code{$\triangle$DEF}.
...
A simple and common OBJ format model can be loaded as an extra model for a \code{Placemark}. 


There are some simple Wavefront OBJ models are created by the Blender (free software) and being used in this application.


A simple OBJ parser is created only support v (vertex indices), vn (vertex normals), fv (face vertex), fvn (face vertex normals), and MTL syntax is ignored \cite{shen.obj-parser.2011}.
