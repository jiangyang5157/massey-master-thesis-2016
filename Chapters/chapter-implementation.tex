...
A Path is for the server to guarantee the latest data (if any) will be pushed to each client (Android smartphone). It server as a compressed ZIP file, and it contains one or more files for client to update. Patch validation is happening whenever the app starts. First, the client sent requests for the patch file $http://xxx.xxx.xxx.xxx:8080/assets/static.zip$ from the file server. Before actual download the file, take the \code{lastModifiedTime} data of remote Patch from HTTP response headers, and compare it with the other \code{lastModifiedTime} data of local's Patch file. Only when the local's Patch is out of date, the client continues to download the remote Patch file and replacing any existing local files. For a special scenario when the app was just installed in the first time launch, also the network is disconnected. A built-in default Patch that included in the APK (Android app binary) will be uncompressed to avoid no available data. Diagram \ref{fig:patch-check} shows the simplified process.
...
The three delta values of any \code{Placemark} position $P$ in a cell with known box center $O$ are:
...
When the \code{Placemark} is literally land on the axis, the associated cell does not conform to the fact. Because of the initialization of boolean \code{octant[]} value came from the delta values on three axes, the \code{Placemark} only has one associated cell. As we can see from \ref{fig:octree-associated-cell}, ray $R_1$ and $R_2$ should be both considered intersecting with the \code{Placemark}. But, ray $R_2$ will be invalid due to it did not intersect with the associated cell $0$.

\[
\begin{array}{lr}
\code{For each axis:}\\
\;\;\;\;\;\;\;\;\code{float delta = obj.center[i] - box.center[i]}\\
\;\;\;\;\;\;\;\;\code{octant[i] = delta >= 0}\\
\end{array}
\]
...
\[
\begin{array}{lr}
\code{const float reciprocalScaleOnScreen = 0.01;}\\
\code{float w = (mvp * vec4(0, 0, 0, 1)).w * reciprocalScaleOnScreen;}\\
\code{vec4 position = vec4(a\_Position.xyz * w , 1);}\\
\end{array}
\]
...
From the Table \ref{tab:opengl-compute}, the calculation for final projected vertex in camera space can be expressed as:
...
\]
f
But, the expression of position in the camera space for ray-point can be simplified as:
...
Finally, the ray intersect with the point only if:

\begin{equation}
\begin{array}{lr}
\begin{aligned}
\label{equ:ray-point-intersection}
