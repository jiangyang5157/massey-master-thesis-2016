...
\begin{figure}[H]
\caption{Default OpenGL coordinate system (left) mapped to a typical Android device screen (right) \parencite{google.opengles.2016}}
...
\begin{figure}[H]
\caption{Icosahedron rectangles \parencite{wiki.icosahedron-rectangles.2006}}
...
\begin{figure}[H]
\caption{earth-centered, earth-fixed \parencite{wiki.ecef.2016}}
\label{fig:ecef}
\centering
...
In order to get an extracted content from a Wikipedia page, we can transform the URL to a Wiki-API based open-search URL \parencite{wiki.api.2016}, which will return a json format raw data that we can easily get what we need from different json tags.
